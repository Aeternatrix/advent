// ====================================================================
//  $File: main.c $
//  $Date: 06-07-2025 @ 18-15-34 $
//  $Revision: 0.0.0 $
//  $Creator: aeternatrix $
//  $Notice: (C) Copyright 2025 by aeternatrix. All Rights Reserved. $
// ===================================================================

// This time's input is just so perfectly basically already usable, so...
// this time, no character readings! Woo!
//
...
#include <stdio.h>
typedef struct { int capacity, durability, flavour, texture, calories; } ingredient;
        ...
        for (int b = 0; b <= 100; b++) {
            for (int c = 0; c <= 100; c++) {
                for (int d = 0; d <= 100; d++) {
                    ...
                    maximum = max(maximum, calc_score(a, b, c, d));
                }
            }
        }
    }
    ...
    for (int a = 0; a <= 100; a++) {
        for (int b = 0; b <= 100; b++) {
            for (int c = 0; c <= 100; c++) {
                for (int d = 0; d <= 100; d++) {
                    if (a+b+c+d != 100) continue;
                    if (a*Sprinkles.calories + b*Butterscotch.calories + c*Chocolate.calories + d*Candy.calories != 500) continue;
                    maximum = max(maximum, calc_score(a, b, c, d));
                }
            }
        }
